...
const useNotifications = () => {
    ...
    const markAllAsRead = async () => {
        ...
        if (unreadIds.length === 0) {
            addToast({ title: "No unread notifications." });
            return;
        }
...
export default function NotificationsPage() {
    ...
    const handleViewClick = (notification: Notification) => {
        ...
        if (notification.relatedLink) {
            ...
            addToast({ title: "No Link Available", description: "No specific context link for this notification." });
        }
    }
                ...
                <div className="p-4 space-y-4">
                    {loading ? (
                        ...
                        notifications.map((notification) => (
                            <div
                                key={notification.id}
                                className={`flex items-start gap-4 p-4 rounded-lg border dark:border-gray-700 transition-colors ${notification.read
